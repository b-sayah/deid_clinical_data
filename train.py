...
import torchcrf
import model.bilstm_torch_crf as bilstm_torch_crf
...
parser.add_argument('--model', default='net',
                    # type=pathlib.Path, # TODO (as lstm confusion ) rename net.py
                    help="select a model type from lstm, bilstm, or bilstm__torch_crf.")
...
if __name__ == '__main__':
    ...
    if args.model == 'lstm':
        ...
        model = bilstm.BiLstm(params).cuda() if params.cuda else bilstm.BiLstm(params)
    elif args.model == 'bilstm_torch_crf':
        model = bilstm_torch_crf.BiLstm_TorchCrf(params).cuda() if params.cuda else bilstm_torch_crf.BiLstm_TorchCrf(params)
        print("=" * 4, "BILSTM CRF IN USE", "=" * 4)
    else:
        logging.info("select a model type from lstm, bilstm, or bilstm-crf. need to add .py \
                    extension")
        sys.exit()
    ...
    if args.model == 'bilstm_torch_crf':
        print(f"Torch CRF LAYER")
        loss_fn = torchcrf.CRF(num_tags=9, batch_first=True)
